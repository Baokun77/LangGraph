// Automated Evaluator for Agent Performance Testing
  ...
  try {
    ...
    switch (agentName) {
      case 'ReAct':
        ...
        break;
      case 'ToT':
        result = await totApp.invoke({
          messages: [new HumanMessage(question)]
        });
        break;
      case 'Reflect':
        result = await reflectApp.invoke({
          messages: [new HumanMessage(question)]
        });
        break;
      default:
...
function generateSummary(results: EvaluationResult[]) {
  const summary: { [key: string]: any } = {};
  
  // Group by agent and task type
  results.forEach(result => {
    const key = `${result.agent}_${result.taskType}`;
    if (!summary[key]) {
      summary[key] = {
        agent: result.agent,
    ...
    summary[key].total++;
    summary[key].totalTime += result.inferenceTime;
    
    if (result.success) summary[key].success++;
    
    // Type-specific metrics
    if (result.taskType === 'factual' && result.evaluationDetails.citationHit) {
      summary[key].citationHits++;
    }
    if (result.taskType === 'tool' && result.evaluationDetails.toolInvoked) {
      summary[key].toolInvocations++;
    }
    if (result.taskType === 'reasoning' && result.evaluationDetails.reasoningCorrect) {
      summary[key].reasoningPasses++;
    }
  });
...
async function runAutomatedEvaluation() {
  console.log("🚀 Starting Automated Agent Evaluation");
  console.log("=".repeat(60));
  
  // Load tasks
  const tasks = loadTasks('evaluation_dataset.jsonl');
  ...
  const agents = ['ReAct', 'ToT', 'Reflect'];
  ...
  console.log("\n## Performance by Agent and Task Type");
  console.log("\n| Agent | Task Type | Total | Success | Success Rate | Avg Time | Special Metrics |");
  console.log("|-------|-----------|-------|---------|--------------|----------|-----------------|");
  
  Object.values(summary).forEach((stats: any) => {
    const successRate = ((stats.success / stats.total) * 100).toFixed(1);
    const avgTime = (stats.totalTime / stats.total).toFixed(2);
    
    let specialMetrics = '';
    if (stats.taskType === 'factual') {
      specialMetrics = `Citation Hit: ${stats.citationHits}/${stats.total}`;
    } else if (stats.taskType === 'tool') {
      specialMetrics = `Tool Invoked: ${stats.toolInvocations}/${stats.total}`;
    } else if (stats.taskType === 'reasoning') {
      specialMetrics = `Reasoning Pass: ${stats.reasoningPasses}/${stats.total}`;
    }
    
    console.log(`| ${stats.agent} | ${stats.taskType} | ${stats.total} | ${stats.success} | ${successRate}% | ${avgTime}s | ${specialMetrics} |`);
  });
  
  // Overall agent performance
  console.log("\n## Overall Agent Performance");
  console.log("\n| Agent | Total Tasks | Success Rate | Avg Time |");
  console.log("|-------|-------------|--------------|----------|");
  
  const agentStats: { [key: string]: any } = {};
  results.forEach(result => {
    if (!agentStats[result.agent]) {
      agentStats[result.agent] = { total: 0, success: 0, totalTime: 0 };
    }
    agentStats[result.agent].total++;
    agentStats[result.agent].totalTime += result.inferenceTime;
    if (result.success) agentStats[result.agent].success++;
  });
  
  Object.entries(agentStats).forEach(([agent, stats]) => {
    const successRate = ((stats.success / stats.total) * 100).toFixed(1);
    const avgTime = (stats.totalTime / stats.total).toFixed(2);
    console.log(`| ${agent} | ${stats.total} | ${successRate}% | ${avgTime}s |`);
  });
