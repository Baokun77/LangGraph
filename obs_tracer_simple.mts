...
export class ObsTracerSimple {
  ...
  private logNodeExecution(
    ...
    try {
      ...
      // Also write to a simple CSV file for easy viewing
      this.writeToCsv(traceData);
  ...
  private writeToCsv(traceData: TraceData): void {
    const csvLine = [
      traceData.run_id,
      traceData.agent_type,
      traceData.step_idx,
      traceData.node_name,
      `"${(traceData.input_summary || '').replace(/"/g, '""')}"`,
      `"${(traceData.observation_summary || '').replace(/"/g, '""')}"`,
      traceData.tool_name || '',
      traceData.latency_ms,
      traceData.prompt_tokens,
      traceData.completion_tokens,
      traceData.cost_usd,
      `"${(traceData.errors || '').replace(/"/g, '""')}"`,
      traceData.timestamp
    ].join(',') + '\n';
    
    const filePath = join(this.tracesDir, 'observability.csv');
    
    try {
      // Write header if file doesn't exist
      if (!existsSync(filePath)) {
        const header = 'run_id,agent_type,step_idx,node_name,input_summary,observation_summary,tool_name,latency_ms,prompt_tokens,completion_tokens,cost_usd,errors,timestamp\n';
        writeFileSync(filePath, header);
      }
      writeFileSync(filePath, csvLine, { flag: 'a' });
    } catch (error) {
